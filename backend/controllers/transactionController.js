...
export const getTransaction = async (req,res)=>{
    try {
        ...
        const _sevenDayAgo = new Date(today);
        _sevenDayAgo.setDate(today.getDate()-7);
       const sevenDayAgo = _sevenDayAgo.toISOString().split("T")[0];
       ...
       const {userId} = req.body.user;
        ...
        return res.status(200).json({status: "success", data: transactions.rows})
    } catch (error) {
        console.log(error);
...
export const transferMoneyToAccount = async (req,res)=>{
    try {
        ...
        {
            return res.status(404).json({status: "failed",message: "required all field"});
        }
        const newAmount= Number(amount);
        if(isNaN(newAmount) || newAmount<=0)
        {
            return res.status(400).json({status: "failed",message: "Ammount should be greated than 0"});
        }
        ...
        if(!fromAccount)
        {
            return res.status(404).json({status: "failed",message: "Account information not found"});

        }
        if(newAmount > fromAccount.account_balance)
        {
           return res.status(400).json({status: "failed",message: "Transfer failed Insufficient account balance"});
        }
        await pool.query("BEGIN");
        await pool.query({
            text: `UPDATE tblaccount SET account_balance = account_balance - $1 , updatedat=CURRENT_TIMESTAMP WHERE id=$2`,
